    ...
    <div class="grid gap-4 md:grid-cols-[minmax(0,1.75fr)_minmax(140px,0.6fr)_minmax(150px,0.65fr)_minmax(160px,auto)] md:items-center">
      <div class="min-w-0 space-y-2">
        <div class="flex flex-wrap items-center gap-2">
          <p class="text-sm font-semibold leading-5 text-slate-900">{{ apartment.name }}</p>
          <span
            v-if="selected"
      ...
      <div class="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1">
        <span class="text-[9px] font-semibold uppercase tracking-[0.18em] text-slate-400">Inquilino</span>
        <p v-if="occupant" class="text-[13px] font-semibold text-slate-700">{{ occupant.full_name }}</p>
        <span v-else class="inline-flex items-center gap-1 whitespace-nowrap rounded-full bg-slate-100 px-2.5 py-0.5 text-[10px] font-medium text-slate-500">
      ...
      <div>
        <p class="text-[9px] font-semibold uppercase tracking-[0.18em] text-slate-400">Renta</p>
        <p class="mt-1 text-[13px] font-semibold leading-5 text-slate-900">{{ formatCurrency(apartment.monthly_rent) }} / mes</p>
      </div>

      <div>
        <p class="text-[9px] font-semibold uppercase tracking-[0.18em] text-slate-400">Estado</p>
        <div class="mt-1">
          <StatusBadge :status="statusValue" compact />
        </div>
      </div>
